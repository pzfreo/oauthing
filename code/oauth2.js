...
//   console.log("server grant", client);
  ...
  db.authorizationCodes.find(code, function (err, authCode) {
    ...
    db.authorizationCodes.delete(code, function (err, result) {
      ...
      db.accessTokens.save(token, config.token.calculateExpirationDate(), authCode.userID, authCode.clientID, authCode.scope, function (err) {
        ...
        if  (true) { // always provide a refresh token // (authCode.scope && authCode.scope.indexOf("offline_access") === 0) {
...
server.exchange(oauth2orize.exchange.password(function (client, username, password, scope, done) {
  ...
  db.users.findByUsername(username, function (err, user) {
    ...
    db.accessTokens.save(token, config.token.calculateExpirationDate(), user.id, client.id, scope, function (err) {
      ...
      //I mimic openid connect's offline scope to determine if we send
      //a refresh token or not
      // hacked to always produce a refresh token.
      if (true) { // scope && scope.indexOf("offline_access") === 0) {
        refreshToken = utils.uid(config.token.refreshTokenLength);
...
// we need to save the scope in the session as it doesn't belong in the db
server.serializeClient(function (client, done) {
//   console.log("serialize client");
  return done(null, { id: client.id, scope: client.scope} );
});

server.deserializeClient(function (o, done) {
  db.clients.find(o.id, function (err, client) {
    if (err) {
      return done(err);
    }
    client.scope = o.scope;
...
//     console.log("eli"+req.isAuthenticated());
//     console.log(req);
